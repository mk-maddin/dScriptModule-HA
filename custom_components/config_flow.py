...
from .configuration_schema import (
    DSCRIPTMODULE_SCHEMA,
    async_get_OPTIONS_DSCRIPTMODULE_SCHEMA,
)
from .const import (
    DEFAULT_NAME,
    DOMAIN,
    ...
    async def async_step_resource(self, user_input: Optional[Dict[str, Any]] = None):
        ...
        try:
            ...
            if user_input is not None:
                _LOGGER.debug("%s - ConfigFlowHandler: async_step_resource add user_input to data", DOMAIN)                
                self.data=user_input
                return await self.async_step_final()
            return self.async_show_form(step_id=CONF_RESOURCE, data_schema=DSCRIPTMODULE_SCHEMA, errors=errors)
        except Exception as e:
...
class OptionsFlowHandler(config_entries.OptionsFlow):
    ...
    async def async_step_resource(self, user_input=None):
        _LOGGER.debug("%s - OptionsFlowHandler: async_step_resource: %s", DOMAIN, user_input)
        try:
            OPTIONS_DSCRIPTMODULE_SCHEMA = await async_get_OPTIONS_DSCRIPTMODULE_SCHEMA(self.config_entry.data)
            if not user_input:
                return self.async_show_form(step_id=CONF_RESOURCE, data_schema=OPTIONS_DSCRIPTMODULE_SCHEMA)
            _LOGGER.debug("%s - OptionsFlowHandler: async_step_resource - user_input: %s", DOMAIN, user_input)
            #self.data.update(user_input)
            self.hass.config_entries.async_update_entry(self.config_entry, data=user_input, options=self.config_entry.options)            
            _LOGGER.debug("%s - OptionsFlowHandler: async_step_resource complete: %s", DOMAIN, user_input)
            return await self.async_step_final()
        except Exception as e:
            _LOGGER.error("%s - OptionsFlowHandler: async_step_resource failed: %s (%s.%s)", DOMAIN, str(e), e.__class__.__module__, type(e).__name__)
            return self.async_abort(reason="exception")
